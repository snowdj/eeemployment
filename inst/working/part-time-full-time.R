...
cleaned_subset_2016 <- raw_subset_2016 %>%
  ...
  dplyr::mutate(FTPT = as.character(haven::as_factor(FTPT))) %>%
  dplyr::mutate(FTPT = ifelse(is.na(FTPT), "No answer", FTPT))
...
openxlsx::write.xlsx(ftpt, "full_time_part_time.xlsx")
